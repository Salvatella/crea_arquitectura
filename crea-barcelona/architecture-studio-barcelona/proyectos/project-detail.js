(() => {
  ...
  if (year && facts) {
    const location = Array.from(facts.children).find((fact) => fact.querySelector('dt')?.textContent.trim() === 'Location.');
    const yearFact = `<div><dt>Year.</dt><dd>${year}</dd></div>`;
  ...
  if (filename === 'casa-troana.html') {
    ...
    if (topRow && !document.querySelector('.troana-gallery__plan')) {
      topRow.insertAdjacentHTML('afterend', '<div class="troana-gallery__plan"><figure><img loading="lazy" src="../../../imgs/enviament_1_2026-05-09/enviament_1_2025-05-09/crea_web_residencial/2023_casa_troana/troana_planta_baixa.jpg" alt="Ground floor plan of TROANA House"></figure></div>');
    }
  }
